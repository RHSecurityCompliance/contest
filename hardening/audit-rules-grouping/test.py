...
def unselect_all_rules_except_audit(orig_ds, new_ds):
    rule_line = re.compile(r'<.*Rule.*id="xccdf_org.ssgproject.content_rule_')
...
def verify_syscalls_grouped_in_audit_rules(audit_syscalls, audit_rules_file):
    ...
    syscall_patterns = [r'\b' + syscall + r'\b' for syscall in audit_syscalls]
...
try:
    ...
    with open('audit.rules', 'w') as f:
        util.subprocess_run(['auditctl', '-l'], stdout=f, check=True)
finally:
    util.restore('/etc/audit')
    util.subprocess_run(['augenrules', '--load'], check=True)

for group in syscalls_groups:
    if verify_syscalls_grouped_in_audit_rules(group, 'audit.rules'):
        results.report('pass', syscalls_pretty_print(group))
    else:
        results.report('fail', syscalls_pretty_print(group))

results.report_and_exit(logs=['report.html', 'remediation-arf.xml', 'audit.rules'])
