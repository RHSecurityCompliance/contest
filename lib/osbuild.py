...
class Blueprint:
    ...
    def set_openscap_datastream(self, ds_file):
        if '[customizations.openscap]' not in self.assembled:
            raise SyntaxError("openscap section not found")
        # flags=re.M seems broken with '^' and '?' together,
        # it seems to ignore '^' and match like '.*datastream = ...'
        self.assembled = re.sub(
            '\n#? ?datastream = [^\n]+\n',
            '\ndatastream = "{ds_file}"\n',
            self.assembled,
            count=1,
